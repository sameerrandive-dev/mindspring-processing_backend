...
class Quiz(Base):
    ...
    questions: Mapped[dict] = mapped_column(JSONB, default=dict)  # Stores the quiz questions as JSON
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    deleted_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
    metadata_: Mapped[dict] = mapped_column(JSONB, default=dict)
    model: Mapped[str] = mapped_column(String(100), nullable=True)  # Model used to generate the quiz
    version: Mapped[int] = mapped_column(default=1)
    
    # Relationships
    notebook: Mapped["Notebook"] = relationship("Notebook", back_populates="quizzes")
